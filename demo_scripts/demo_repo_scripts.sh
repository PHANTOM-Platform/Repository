...
# 13. ################## TEST OF UPLOADING A FILE WITH A VALID TOKEN, access must be accepted : 200 ##########
	echo -e "\n${LIGHT_BLUE}";
	echo "bash repo_put.sh -t ${newtoken} -sfp "../web/example.h" -sjp "../web/exampleh.json" -dp "main.h" -df "mypath/" -s ${server} -port ${repository_port} ";
...
# 14. ################## TEST OF DOWNLOADING A FILE WITH A VALID TOKEN, access must be accepted : 200 ###### 
	...
	bash repo_get_file.sh -t ${newtoken} -file "main.h" -path "mypath/" -s ${server} -port ${repository_port} 
# 17. ######## TEST OF DOWNLOADING METADATA WITH A VALID TOKEN for files in a path, access must be accepted : 200 ########
	...
	bash repo_get_metadata.sh -t ${newtoken} -file "main.h" -path "mypath/" -s ${server} -port ${repository_port} 
